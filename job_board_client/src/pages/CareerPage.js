...
export const Careerpage = () => {
    ...
    async function handlesubmit(email){
        ...
        const applyjob = await fetch("https://job-board-server-eo10.onrender.com/jobprofile/addApplicant",{
            method:"PUT",
            headers:{
                            ...
                            .map((profile) => (
                                <>
                                    ...
                                    >
                                        ...
                                        >
                                            <button onClick={() => {setPopup(true);handleApply(profile._id);return}} style={{ borderRadius: '5px', backgroundColor: '#61dafb', padding: '0.5%' }}>Apply</button>
                                        </div>
                                    </div>
                                    <div
                                        style={popup ? {
                                    ...
                                    >
                                        <div style={{ display: 'flex', justifyContent: 'space-between',padding:'1%' }}>
                                            <h5>To apply to this Job applicant profile should be filled. </h5>
                                            <h3 style={{ border: '2px solid red', color: "red" }} onClick={() => setPopup(false)}>X</h3>
                                        </div>
                                        <div style={{display:'flex',width:'100%',padding:'1%'}}>
                                            {
                                               msg.length>0 ?(
                                                <div>
                                                    The following details {msg} are required for applying to the job role. to update 
                                                    <span onClick={()=>navigate('/user')} style={{color:'#61dafb'}}>Click here.</span>
                                                </div>
                                               ):(
                                                <div style={{width:'100%'}}>
                                                    Review Application:-<br/>
                                                    Name:-{userprofiledata.fullname}<br/>
                                                    Email:-{userprofiledata.email}<br/>
                                                    Dateofbirth:-{userprofiledata.dob}<br/>
                                                    Phone:-{userprofiledata.phone}<br/>
                                                    description:-{userprofiledata.description}<br/>
                                                    <div style={{display:'flex',justifyContent:'space-between',width:'100%'}}>
                                                        <button onClick={()=>navigate('/user')} style={{backgroundColor:'#61dafb', borderRadius:'5px',padding:'1%'}}>update Profile</button>
                                                        <button onClick={()=>handlesubmit(profile._id,userprofiledata.email)} style={{backgroundColor:'#61dafb', borderRadius:'5px',padding:'1%'}}>submit Profile</button>
                                                    </div>
                                                </div>
                                               ) 
                                            }
